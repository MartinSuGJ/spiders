    ...
    def process_item(self, item, spider):
        if isinstance(item, TweetItem):
            ...
            if dbItem:
                ...
                logger.info("Mongo: Add tweet:%s"%item['url'])
            ...
            if dbItem:
                ...
                logger.info("Mongo: Add user:%s"%item['screen_name'])

        else:
            logger.info("Mongo: Item type is not recognised! type = %s"%type(item))
    ...
    def process_item(self, item, spider):
        if isinstance(item, TweetItem):
            ...
            if os.path.isfile(savePath):
                ...
                self.save_to_file(item,savePath)
                logger.info("File: Add tweet:%s"%item['url'])
            ...
            if os.path.isfile(savePath):
                ...
                self.save_to_file(item,savePath)
                logger.info("File: Add user:%s"%item['screen_name'])

        else:
            logger.info("File: Item type is not recognised! type = %s"%type(item))
